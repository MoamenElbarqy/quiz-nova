...
public static class QuizEndpoints
{
    public static IEndpointRouteBuilder MapQuizEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        .CacheOutput(policy => policy.Tag("quizzes"))
        .RequireAuthorization(new AuthorizeAttribute { Roles = $"{nameof(UserRole.Admin)},{nameof(UserRole.Instructor)}" })
        ...
        quizzesGroup.MapPut("{quizId:guid}/metadata", async (ISender sender, Guid quizId, UpdateQuizMetadataRequest request) =>
        {
        ...
        quizzesGroup.MapPost("{quizId:guid}/questions", async (ISender sender, Guid quizId, CreateQuizQuestionRequest request) =>
        {
        ...
        quizzesGroup.MapPut("{quizId:guid}/questions/{questionId:guid}", async (ISender sender, Guid quizId, Guid questionId, UpdateQuestionRequest request) =>
        {
        ...
        quizzesGroup.MapPut("{quizId:guid}/course", async (ISender sender, Guid quizId, UpdateQuizCourseIdRequest request) =>
        {
        ...
        .WithDescription("Changes the course associated with a quiz. This is a destructive operation that clears all existing questions.")
        .RequireAuthorization(new AuthorizeAttribute { Roles = nameof(UserRole.Instructor) })
        .Produces(StatusCodes.Status204NoContent)
        .ProducesProblem(StatusCodes.Status400BadRequest)
        .ProducesProblem(StatusCodes.Status403Forbidden)
        ...
        quizzesGroup.MapDelete("{quizId:guid}/questions/{questionId:guid}", async (ISender sender, Guid quizId, Guid questionId) =>
        {
