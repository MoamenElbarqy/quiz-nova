  ...
  template: `
    <section class="charts-grid" aria-label="Course analytics">
      <article class="chart-card">
        ...
        <div class="chart-container">
          @defer (on viewport({rootMargin: '100px'}); prefetch on viewport({rootMargin: '200px'})) {
            <p-chart type="bar" [data]="enrolledChartData()" [options]="enrolledChartOptions()" height="300" />
          } @placeholder {
      ...
      <article class="chart-card">
        ...
        <div class="chart-container">
          @defer (on viewport({rootMargin: '100px'}); prefetch on viewport({rootMargin: '200px'})) {
            <p-chart type="bar" [data]="performanceChartData()" [options]="performanceChartOptions()" height="300" />
          } @placeholder {
            <app-chart-placeholder />
          }
        </div>
...
export class InstructorDashboardCharts {
  ...
  protected readonly performanceChartData = computed(() => {
    ...
    const backgroundColors = perf.map(c => 
      c.avgScore >= 80 ? chartColor('--clr-green-100') :
      c.avgScore >= 50 ? chartColor('--clr-gray-100') :
      chartColor('--clr-red-50')
    );
    const borderColors = perf.map(c => 
      c.avgScore >= 80 ? chartColor('--clr-green-400') :
      c.avgScore >= 50 ? chartColor('--clr-gray-600') :
      chartColor('--clr-red-500')
    );
    const hoverBackgroundColors = perf.map(c => 
      c.avgScore >= 80 ? chartColor('--clr-green-200') :
      c.avgScore >= 50 ? chartColor('--clr-gray-200') :
