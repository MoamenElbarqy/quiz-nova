      ...
      return {
            ...
            return forkJoin({
              ...
              catchError((err) => {
                patchState(
                  ...
                  setError(
                    'loadCourse',
                    getApiErrorMessage(err, 'Failed to load course management data.'),
                  ),
                );
                return EMPTY;
              }),
        ...
        updateInstructor: rxMethod<{ instructorId: string; onSuccess?: () => void }>(
          exhaustMap(({ instructorId, onSuccess }) => {
            ...
            return coursesService.updateCourseInstructor(previousCourse.id, { instructorId }).pipe(
              tap((updatedCourse) => {
                patchState(store, { course: updatedCourse }, setFulfilled('updateInstructor'));
                onSuccess?.();
              }),
              catchError((err) => {
                const errorMessage = getApiErrorMessage(err, 'Failed to update instructor.');
                patchState(
                  store,
                  {
                    course: previousCourse,
                  },
                  setError('updateInstructor', errorMessage),
                );
                return EMPTY;
              }),
            );
        ...
        enrollStudent: rxMethod<{ studentId: string; onSuccess?: () => void }>(
          exhaustMap(({ studentId, onSuccess }) => {
            ...
            patchState(
              store,
              {
                availableStudents: previousAvailableStudents.filter(
            ...
            return enrollmentService.enrollStudent(course.id, studentId).pipe(
              tap(() => {
              ...
              catchError((err) => {
                patchState(
                  ...
                  setError('enrollStudent', getApiErrorMessage(err, 'Failed to enroll student.')),
                );
                return EMPTY;
              }),
            );
        ...
        removeStudent: rxMethod<{ studentId: string; onSuccess?: () => void }>(
          exhaustMap(({ studentId, onSuccess }) => {
            ...
            return enrollmentService.removeStudent(course.id, studentId).pipe(
              tap(() => {
              ...
              catchError((err) => {
                patchState(
                  ...
                  setError('removeStudent', getApiErrorMessage(err, 'Failed to remove student.')),
                );
                return EMPTY;
              }),
            );
